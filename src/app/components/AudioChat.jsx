...
export async function sendToGPT(audioBlob) {
  // Get audioBlob from our context
//   const { audioBlob } = useContext(AppContext);
  ...
  try {
    ...
    if (data.audioData) {
      ...
      // If there's only a text response, log it
...
// Helper function to decode and play audio from base64 data
function playAudioFromBase64(audioBase64) {
  try {
    ...
    // The API returns PCM16 format, so we need to wrap it in a WAV container
...
/**
 * Creates a WAV file from PCM16 audio data
 * @param {Uint8Array} pcmData - The PCM16 audio data
